...
- Leave the update notification without any input, then return. Open release notes and confirm the notification remains, reports the browser action and uses the extended duration. Dismiss it and confirm the sidebar and Updates page still offer the update.
...
- Open matching, newer and older setup versions. Verify maintenance choices, primary Update for newer setup, downgrade prevention and continued local maintenance when offline.
